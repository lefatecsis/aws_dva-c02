const originalQuestions = [
  ...
  },
  {
    ...
    "options": [
      "\"HelloWorld\": { \"Type\": \"Task\", \"Resource\": \"arn:aws:lambda:us-east-1:123456789012:function:HelloFunction\", \"Next\": \"AfterHelloWorldState\", \"Comment\": \"Executa a função Lambda HelloWorld\" }",
  ...
  },
  {
    ...
    "correct": 3,
    "detailedExplanations": [
      "Correta. Este é um tipo de parâmetro válido que representa o nome de um par de chaves do Amazon EC2, permitindo a validação automática do valor informado.",
      "Correta. Este tipo de parâmetro é válido e representa uma lista de strings literais separadas por vírgulas, útil para passar múltiplos valores em um único parâmetro.",
      "Correta. Este é um tipo de parâmetro válido que aceita uma string literal, sendo o tipo mais básico e comum para parâmetros em CloudFormation.",
      "Incorreta. Este não é um tipo de parâmetro válido no AWS CloudFormation. Os parâmetros são independentes e não podem depender diretamente uns dos outros, portanto, não existe um tipo chamado DependentParameter."
    ]
  },
  {
  ...
  {
    "question": "Uma equipe de desenvolvimento está trabalhando em uma função AWS Lambda que acessa o DynamoDB. A função Lambda deve realizar um upsert, ou seja, deve recuperar um item e atualizar alguns de seus atributos ou criar o item caso ele não exista. Qual das seguintes opções representa a solução com as permissões IAM MÍNIMAS que podem ser usadas pela função Lambda para alcançar essa funcionalidade?",
    "options": [
    ...
    "correct": 1,
    "detailedExplanations": [
      "Incorreta. A ação dynamodb:GetRecords não existe; o correto seria dynamodb:GetItem para recuperar um item. Além disso, dynamodb:UpdateTable é usada para modificar a estrutura da tabela, não para manipular dados. Portanto, essas permissões não são adequadas nem mínimas para realizar um upsert.",
      "Correta. A ação dynamodb:GetItem permite recuperar o item existente, enquanto dynamodb:UpdateItem permite atualizar atributos de um item existente ou criar um novo item caso ele não exista (upsert). Essas são as permissões mínimas necessárias para a função Lambda realizar o upsert de forma eficiente e segura.",
      "Incorreta. A ação dynamodb:AddItem não existe na API do DynamoDB. A ação correta para inserir ou atualizar um item é dynamodb:PutItem ou dynamodb:UpdateItem. Portanto, essa opção contém uma permissão inválida e está incorreta.",
      "Incorreta. Embora essa opção funcione, a permissão dynamodb:PutItem não é necessária para realizar um upsert, pois a ação dynamodb:UpdateItem já cobre a criação e atualização do item. Incluir dynamodb:PutItem adiciona permissões desnecessárias, contrariando o princípio do menor privilégio."
    ]
  },
  {
