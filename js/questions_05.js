const originalQuestions = [
  {
    "question": "Uma empresa líder no setor financeiro oferece serviços de agregação de dados para firmas de trading de Wall Street. A empresa cobra seus clientes com base na quantidade de dados de clickstream fornecidos. Como a empresa opera em um setor regulado, é necessário que os mesmos dados de clickstream, na mesma ordem, estejam disponíveis para auditoria dentro de uma janela de 7 dias. Como um Desenvolvedor Associado, qual dos seguintes serviços AWS você recomendaria para executar o processo de faturamento e auditoria sobre os dados de clickstream fornecidos, garantindo que ambos os processos utilizem os dados na mesma ordem?",
    "options": [
  ...
  {
    "question": "Você trabalha para uma empresa de transporte que está automatizando a criação de clusters ECS com um Auto Scaling Group usando um template AWS CloudFormation que aceita o nome do cluster como parâmetro. Inicialmente, você lançou o template com o valor de entrada 'MainCluster', que implantou cinco instâncias distribuídas em duas zonas de disponibilidade. Na segunda vez, você lançou o template com o valor de entrada 'SecondCluster'. Contudo, as instâncias criadas na segunda execução também foram lançadas no cluster 'MainCluster', mesmo após especificar um nome de cluster diferente. Qual é a causa raiz desse problema?",
    "options": [
  ...
  {
    "question": "Para atender às diretrizes de conformidade, uma empresa precisa garantir a replicação de quaisquer dados armazenados em seus buckets S3. Quais das seguintes características estão corretas ao configurar um bucket S3 para replicação? (Selecione três.)",
    "options": [
  ...
  {
    "question": "Uma grande empresa armazena seus dados estáticos em buckets do Amazon S3. Cada linha de serviço da empresa possui sua própria conta AWS. Para um caso de uso de negócios, o departamento Financeiro precisa conceder acesso aos dados do bucket S3 deles para o departamento de Recursos Humanos. Qual das opções abaixo NÃO é viável para acesso cross-account a objetos de buckets S3?",
    "options": [
  ...
  {
    "question": "Um novo membro da sua equipe está trabalhando na criação de uma Dead Letter Queue (DLQ) para funções AWS Lambda. Como um Associate Developer, você pode ajudá-lo a identificar os casos de uso em que o AWS Lambda adicionará uma mensagem em uma DLQ após o processamento? (Selecione duas.)",
    "options": [
  ...
  {
    "question": "Uma empresa de marketing digital hospeda seu site em um bucket Amazon S3 A. A equipe de desenvolvimento percebe que as fontes web hospedadas em outro bucket S3 B não estão carregando no site. Qual das seguintes soluções pode ser usada para resolver esse problema?",
    "options": [
  ...
  {
    "question": "Uma equipe está avaliando a viabilidade de usar AWS Step Functions para criar um fluxo de trabalho bancário para aprovações de empréstimos. A aplicação web também terá uma etapa de aprovação humana dentro do fluxo. Como um desenvolvedor associado, quais das seguintes características você identificaria como principais para o AWS Step Functions? (Selecione duas.)",
    "options": [
  ...
  {
    "question": "Uma empresa de TI migrou para uma arquitetura serverless na nuvem AWS, onde a camada de computação é implementada por funções Lambda. Os gerentes de engenharia gostariam de monitorar ativamente e solucionar quaisquer falhas nas funções Lambda. Como um Desenvolvedor Associado, qual das seguintes soluções você recomendaria para esse caso de uso?",
    "options": [
  ...
  {
    "question": "Uma empresa de cibersegurança está executando um backend serverless com vários fluxos de trabalho que demandam alta capacidade computacional, rodando em funções Lambda. A equipe de desenvolvimento percebeu uma lentidão no desempenho após analisar as métricas das funções Lambda. Como um Associate Developer, qual das seguintes opções você sugeriria como a MELHOR solução para lidar com esses fluxos de trabalho que exigem alta capacidade computacional?",
    "options": [
  ...
  {
    "question": "Sua equipe mantém um API Gateway público que é acessado por clientes de outro domínio. O uso tem sido consistente nos últimos meses, mas recentemente mais que dobrou. Como resultado, seus custos aumentaram e você gostaria de impedir que outros domínios não autorizados acessem sua API. Qual das seguintes ações você deve tomar?",
    "options": [
  ...
  {
    "question": "Uma empresa de negociação de ações de alta frequência está migrando suas filas de mensagens de sistemas de middleware orientados a mensagens autogerenciados para o Amazon SQS. A equipe de desenvolvimento da empresa deseja minimizar os custos do uso do SQS. Como um Associate Developer, qual das seguintes opções você recomendaria para atender a esse caso de uso?",
    "options": [
  ...
  {
    "question": "Uma empresa possui uma carga de trabalho que requer 14.000 IOPS consistentes para dados que devem ser duráveis e seguros. Os padrões de conformidade da empresa determinam que os dados devem estar protegidos em todas as etapas do seu ciclo de vida em todos os volumes EBS utilizados. Quais das seguintes afirmações são verdadeiras em relação à segurança dos dados no EBS?",
    "options": [
  ...
  {
    "question": "Um desenvolvedor está projetando um template AWS CloudFormation para implantar instâncias Amazon EC2 em várias contas AWS. O desenvolvedor precisa permitir a seleção de instâncias EC2 a partir de uma lista de tipos de instância previamente aprovados. Quais medidas o desenvolvedor pode tomar para integrar a lista de tipos de instância autorizados no template CloudFormation?",
    "options": [
  ...
  {
    "question": "Uma equipe de desenvolvimento implantou uma API REST no Amazon API Gateway em dois estágios diferentes — um estágio de teste e um estágio de produção. O estágio de teste é usado como uma versão para testes e o estágio de produção como uma versão estável. Após as atualizações serem aprovadas nos testes, a equipe deseja promover o estágio de teste para o estágio de produção. Qual das alternativas a seguir representa a solução ideal para esse caso de uso?",
    "options": [
  ...
  {
    "question": "A equipe de desenvolvimento de uma empresa de mídia social está considerando usar o Amazon ElastiCache para aumentar o desempenho dos seus bancos de dados existentes. Como um Associate Developer, quais dos seguintes casos de uso você recomendaria como o MELHOR ajuste para o ElastiCache? (Selecione duas.)",
    "options": [
  ...
  {
    "question": "Você possui uma aplicação web em três camadas composta por uma camada web usando AngularJS, uma camada de aplicação utilizando o AWS API Gateway e uma camada de dados em um banco de dados Amazon Relational Database Service (RDS). Sua aplicação permite que visitantes consultem filmes populares do passado. A empresa deseja reduzir o número de chamadas feitas ao endpoint e melhorar a latência da API. O que você pode fazer para melhorar o desempenho?",
    "options": [
  ...
  {
    "question": "Um estagiário em uma empresa de TI está começando a trabalhar com a AWS Cloud e quer entender a seguinte política de acesso a um bucket do Amazon S3: (trecho de política IAM) Como um Associate Developer, você pode ajudá-lo a identificar para que serve essa política?",
    "options": [
  ...
  {
    "question": "Sua empresa utiliza um Application Load Balancer para rotear o tráfego de usuários finais para aplicações hospedadas em instâncias Amazon EC2. As aplicações capturam informações das requisições recebidas e armazenam esses dados no Amazon Relational Database Service (RDS) utilizando mecanismos Microsoft SQL Server. Como parte de novas regras de conformidade, é necessário capturar o endereço IP do cliente. Como você deve proceder para obter essa informação? (Selecione duas.)",
    "options": [
      "Utilize o cabeçalho X-Forwarded-For.",
      "Você pode configurar o Amazon RDS para capturar automaticamente o endereço IP de origem das requisições feitas pelas aplicações.",
    ...
    "detailedExplanations": [
      ...
      "Incorreta. O Amazon RDS não possui a capacidade de capturar automaticamente o endereço IP do cliente que originou a requisição à aplicação. Ele registra conexões ao banco, mas não armazena o IP do usuário final que acessou a aplicação via ALB. A captura do IP do cliente deve ser feita pela aplicação utilizando cabeçalhos HTTP como X-Forwarded-For.",
  ...
  {
    "question": "Você possui um processo de workflow que obtém código do AWS CodeCommit e faz deploy em instâncias EC2 associadas à tag de grupo ProdBuilders. Você deseja configurar as instâncias para armazenar no máximo duas revisões da aplicação para conservar espaço em disco. Qual das alternativas a seguir permitirá implementar essa configuração?",
    "options": [
  ...
  {
    "question": "Uma empresa utiliza o Amazon RDS como seu banco de dados. Para melhorar a experiência do usuário, foi decidido que uma camada de cache altamente confiável e totalmente gerenciada deve ser configurada na frente do RDS. Qual das opções a seguir é a escolha correta, considerando que a regeneração do conteúdo do cache é uma atividade custosa?",
    "options": [
  ...
  {
    "question": "Uma empresa do setor financeiro deseja garantir que os dados dos clientes estejam sempre criptografados no Amazon S3, mas quer uma solução gerenciada pela AWS que permita controle total para criar, rotacionar e remover as chaves de criptografia. Como um Desenvolvedor Associado, qual das seguintes opções você recomendaria para atender a esse caso de uso?",
    "options": [
  ...
  {
    "question": "Sua empresa planeja abandonar a reserva de instâncias EC2 e deseja adotar uma arquitetura serverless mais ágil. Qual das opções a seguir representa a forma mais simples e com menor esforço para implantar contêineres Docker nessa arquitetura serverless?",
    "options": [
  ...
  {
    "question": "Um desenvolvedor está trabalhando em uma função AWS Lambda que lê dados de objetos do Amazon S3 e grava esses dados em uma tabela do Amazon DynamoDB. Embora a função seja acionada com sucesso a partir de uma notificação de evento do S3 após a criação do objeto, ela encontra uma falha ao tentar gravar os dados na tabela DynamoDB. Qual é a provável causa da falha?",
    "options": [
  ...
  {
    "question": "Um desenvolvedor deseja habilitar o rastreamento do X-Ray em um servidor Linux on-premises que executa uma aplicação personalizada e é acessado por meio do Amazon API Gateway. Qual é a solução mais eficiente que exige a configuração mínima?",
    "options": [
  ...
  {
    "question": "Você migrou um banco de dados SQL Server on-premises para um banco de dados Amazon Relational Database Service (RDS) conectado a uma VPC dentro de uma sub-rede privada. Além disso, a aplicação Java relacionada, que estava hospedada on-premises, foi movida para uma função AWS Lambda. Qual das seguintes opções você deve implementar para conectar a função AWS Lambda à sua instância RDS?",
    "options": [
  ...
  {
    "question": "Você está criando um aplicativo móvel que precisa acessar o AWS API Gateway. Os usuários devem se registrar primeiro antes de poderem acessar sua API, e você deseja que o gerenciamento de usuários seja totalmente gerenciado. Qual opção de autenticação você deve usar para a camada do API Gateway?",
    "options": [
  ...
  {
    "question": "Dois policies estão anexadas a um usuário IAM. A primeira policy declara que o usuário tem explicitamente negado todo acesso a instâncias EC2. A segunda policy declara que o usuário tem permissão para a ação EC2:Describe. Quando o usuário tenta executar a ação 'Describe' em uma instância EC2 usando a CLI, qual será o resultado?",
    "options": [
  ...
  {
    "question": "Uma equipe de desenvolvimento utiliza buckets Amazon S3 compartilhados para fazer upload de arquivos. Devido a esse acesso compartilhado, os objetos nos buckets S3 possuem diferentes proprietários, o que dificulta o gerenciamento dos objetos. Como um desenvolvedor associado, qual das opções a seguir você sugeriria para automaticamente tornar o proprietário do bucket S3 também o proprietário de todos os objetos no bucket, independentemente da conta AWS utilizada para fazer o upload dos objetos?",
    "options": [
  ...
  {
    "question": "Uma aplicação bancária precisa enviar alertas e notificações em tempo real com base em atualizações dos serviços backend. A empresa deseja evitar a implementação de mecanismos complexos de polling para essas notificações. Qual dos seguintes tipos de APIs suportados pelo Amazon API Gateway é a opção mais adequada?",
    "options": [
      "APIs REST",
      "APIs REST ou HTTP",
      "APIs HTTP",
  ...
  },
  {
    ...
    "options": [
      ...
      "Usar o AWS CodePipeline para automatizar a troca de instâncias EC2 manualmente entre as versões da aplicação."
    ],
    "correct": [0, 1],
    "detailedExplanations": [
      ...
      "Incorreta. Embora o AWS CodePipeline possa orquestrar pipelines de CI/CD, ele não realiza diretamente o redirecionamento de tráfego entre ambientes ou substituição manual de instâncias. Essa funcionalidade é obtida por meio de estratégias de implantação oferecidas pelo CodeDeploy, como Blue/Green ou Imutável. Automatizar trocas manuais de instâncias EC2 vai contra o objetivo de usar CodeDeploy para implantações automatizadas e seguras."
    ]
  },
  {
    "question": "A equipe de desenvolvimento de uma empresa de mídia está trabalhando para garantir a segurança de seus bancos de dados. Quais dos seguintes mecanismos de banco de dados da AWS podem ser configurados com Autenticação de Banco de Dados via IAM? (Selecione duas.)",
    "options": [
      "RDS PostgreSQL",
      "RDS Db2",
      "RDS Oracle",
  ...
  {
    "question": "A equipe de desenvolvimento de uma empresa deseja criptografar um objeto de 111 GB utilizando o AWS KMS. Qual das opções a seguir representa a melhor solução?",
    "options": [
  ...
  {
    "question": "O AWS CloudFormation ajuda a modelar e provisionar todos os recursos de infraestrutura em nuvem necessários para o seu negócio. Quais dos seguintes serviços dependem do CloudFormation para provisionar recursos? (Selecione duas.)",
    "options": [
  ...
  {
    "question": "Como desenvolvedor, você deseja criar uma configuração personalizada para instâncias Amazon EC2 executando em um grupo de Auto Scaling. A solução deve permitir que o grupo faça o auto scaling com base na métrica de 'uso médio de RAM' das suas instâncias Amazon EC2. Qual opção oferece a melhor solução?",
    "options": [
  ...
  {
    "question": "Uma empresa adota práticas de desenvolvimento colaborativo. O gerente de engenharia deseja isolar o esforço de desenvolvimento configurando simulações dos componentes da API pertencentes a várias equipes de desenvolvimento. Qual tipo de integração de API é mais adequado para esse requisito?",
    "options": [
      "AWS_PROXY",
      "HTTP",
      "HTTP_PROXY",
  ...
  {
    "question": "Um desenvolvedor da sua equipe configurou o balanceador de carga para distribuir o tráfego igualmente entre instâncias ou entre Zonas de Disponibilidade. No entanto, o Elastic Load Balancing (ELB) está roteando mais tráfego para uma instância ou Zona de Disponibilidade do que para as outras. Por que isso está acontecendo e como pode ser corrigido? (Selecione duas.)",
    "options": [
  ...
  {
    "question": "Uma aplicação roda em uma instância EC2 e processa pedidos diariamente à noite. Essa instância EC2 precisa acessar os pedidos que estão armazenados no S3. Como você recomendaria que a instância EC2 acessasse os pedidos de forma segura?",
    "options": [
  ...
  {
    "question": "Você possui um grupo de Auto Scaling configurado com capacidade mínima de 1 e máxima de 5, projetado para lançar instâncias EC2 em 3 Zonas de Disponibilidade. Durante um período de baixa utilização, uma Zona de Disponibilidade inteira ficou indisponível e sua aplicação sofreu downtime. O que você pode fazer para garantir que sua aplicação permaneça altamente disponível?",
    "options": [
  ...
  {
    "question": "Você é um desenvolvedor que utiliza a AWS CLI para criar funções Lambda que contêm variáveis de ambiente. Suas funções precisarão de mais de 50 variáveis de ambiente contendo informações sensíveis, como nomes de tabelas de banco de dados. Qual é o tamanho total permitido e o número máximo de variáveis de ambiente que podem ser criadas para uma função AWS Lambda?",
    "options": [
  ...
  {
    "question": "Como Desenvolvedor Sênior, você gerencia 10 instâncias Amazon EC2 que realizam muitas requisições de leitura para um banco de dados Amazon RDS para PostgreSQL. Você precisa tornar essa arquitetura resiliente para recuperação de desastres. Quais das seguintes funcionalidades ajudarão a preparar essa arquitetura para recuperação de desastres no banco de dados? (Selecione duas.)",
    "options": [
  ...
  {
    "question": "Um provedor de serviços de telecomunicações armazena seus dados críticos de clientes no Amazon Simple Storage Service (Amazon S3). Quais das seguintes opções podem ser usadas para controlar o acesso aos dados armazenados no Amazon S3? (Selecione duas.)",
    "options": [
